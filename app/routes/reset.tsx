...
export const action: ActionFunction = async ({ request }) => {
  ...
  const passwordError =
    ...
    UserUtils.validatePasswords(password, passwordConfirm);
  if (passwordError) {
    return badRequest({
      error: passwordError,
      fields,
...
export default function ResetRoute() {
  ...
  const [email] = useState(
    actionData?.fields.email ?? new URLSearchParams(search).get("e") ?? ""
  );
  const [verifyToken] = useState(
    actionData?.fields.email ?? new URLSearchParams(search).get("t") ?? ""
  );
    ...
    <div>
      <div>
        <div className="min-h-screen flex flex-col justify-center py-12 px-6 lg:px-8">
          ...
          <div>
            <h2 className="mt-6 text-center text-md leading-9 font-bold text-gray-900 dark:text-slate-300">
              ...
              <div className="max-w-md w-full mx-auto rounded-sm px-8 pt-6 pb-8 mb-4 mt-8">
                <Form method="post">
                  <div>
                    ...
                    <label
                      htmlFor="email"
                      className="block text-sm font-medium leading-5 text-gray-900 dark:text-slate-300"
                    >
